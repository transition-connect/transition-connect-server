...
describe('Integration Tests for sending an email with a login password', function () {
    ...
    it('Successful login', function (done) {
        request(app).post('/login').send({username: 'useR1@irgendwo.ch', password: '123'}).expect(200).end(done);
    });

    it('Login failed because of not existing admin', function (done) {
        request(app).post('/login').send({username: 'useR10@irgendwo.ch', password: '123'}).expect(400).end(done);
    });

    it('Login failed because of invalid password', function (done) {
        request(app).post('/login').send({username: 'useR1@irgendwo.ch', password: '124'}).expect(400).end(done);
    });

    it('Login failed because password has been expired', function (done) {
        request(app).post('/login').send({username: 'useR2@irgendwo.ch', password: '123'}).expect(400).end(done);
    });
});
