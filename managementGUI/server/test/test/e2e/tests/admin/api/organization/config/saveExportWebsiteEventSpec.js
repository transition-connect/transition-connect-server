...
describe('Saving export config for events of an organization imported from a project website', function () {
    ...
    it('Activate export of events with non existing rules', async function () {
        ...
        let nps = await db.cypher().match(`(org:Organization {organizationId: '2'})-[:WEBSITE_EVENT]->(event:Event)
    ...
    it('Deactivate previously exported events', async function () {
        ...
        nps = await db.cypher().match(`(org:Organization {organizationId: '2'})-[:WEBSITE_EVENT]->(event:Event)
